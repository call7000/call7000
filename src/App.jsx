...
export default function App() {
  ...
  function assignMember(memberId, vehicleId) {
    setMembers(ms => ms.map(m => m.id === memberId ? { ...m, vehicleId } : m));
    ...
    try {
      ...
      if (alarmType === "beep") {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          osc1.type = "sawtooth";
          osc1.frequency.setValueAtTime(220, now + offset);
          ...
          osc2.type = "sawtooth";
          osc2.frequency.setValueAtTime(880, now + offset + 0.2);
        ...
        for (let i = 0; i < 2; i++) {
          ...
          osc.frequency.setValueAtTime(1100, now + offset + 0.35);
        ...
        // 東京消防庁風：ピーポーピーポー（960Hz↔770Hz 正弦波）
        for (let i = 0; i < 3; i++) {
          const offset = i * 0.7;
          // ピー（高）
          const osc1 = ctx.createOscillator(); const gain1 = ctx.createGain();
          osc1.connect(gain1); gain1.connect(ctx.destination);
          osc1.type = "sine";
          osc1.frequency.setValueAtTime(960, now + offset);
          ...
          osc2.connect(gain2); gain2.connect(ctx.destination);
          osc2.type = "sine";
          osc2.frequency.setValueAtTime(770, now + offset + 0.35);
  ...
  function getAlarmDuration() {
    const durations = { beep: 900, siren: 1800, urgent: 950, red: 1650, alarm: 1800, yelp: 1450, tfd: 2150 };
    return (durations[alarmType] || 1500) + 500; // 音の長さ + 0.5秒
  }
  ...
  function startAlert() {
    ...
    setTimeout(() => {
      playAlertSound();
      const interval = getAlarmDuration();
      alertTimer.current = setInterval(() => playAlertSound(), interval);
    }, 50);
  }
  function stopAlert() {
    setAlert(false);
  ...
  function closeModal() {
    ...
    setDispatchNote(""); setDispatchLocation("");
    setLocationLoading(false);
  }

  function getCurrentLocation() {
    if (!navigator.geolocation) {
      alert("位置情報がこのブラウザでは使えません");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        alert("位置情報の取得に失敗しました。設定で位置情報を許可してください。");
        setLocationLoading(false);
      },
  ...
  function sendDispatch(typeId) {
    ...
    setVehicleStatuses(s => ({ ...s, [modalVehicleId]: action.nextStatus }));
    setMessages(m => [...m, {
      id: Date.now(), vehicleId: modalVehicleId, type: typeId, text,
      location: loc || null, time: timeStr(), date: datStr()
    }]);
  ...
  function sendChat() {
    ...
    setMessages(m => [...m, {
      id: Date.now(), vehicleId: chatVehicle, type: "chat",
      text: chatInput.trim(), time: timeStr(), date: datStr(), location: null
    }]);
    setChatInput("");
  }
  ...
  return (
    <div style={{
      ...
      {/* ── Alert Overlay */}
      {alert && (
        <div onClick={stopAlert} style={{
          position: "fixed", inset: 0, zIndex: 200,
          display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
          cursor: "pointer", animation: "flash 0.5s infinite",
        }}>
      ...
      {/* ── Header */}
      <div style={{
        padding: "14px 18px 10px", background: "#ffffff",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap", justifyContent: "flex-end" }}>
            {vehicles.map(v => {
              const st = vehicleStatuses[v.id];
              const active = ACTIVE_STATUSES.includes(st);
              const returning = st === "RETURNING";
              if (active) return (
                <div key={v.id} style={{
                  background: v.color, color: "#fff", fontSize: 10, fontWeight: 700,
                  padding: "2px 8px", borderRadius: 20, display: "flex", alignItems: "center", gap: 4
                }}>
                  <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#fff", opacity: pulse ? 1 : 0.3, transition: "opacity 0.3s", display: "inline-block" }} />
                  {v.icon} 出動中
                </div>
              );
              if (returning) return (
            ...
            {vehicles.every(v => !ACTIVE_STATUSES.includes(vehicleStatuses[v.id]) && vehicleStatuses[v.id] !== "RETURNING") && (
              <div style={{ background: "#f0fdf4", color: "#16a34a", fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, border: "1px solid #86efac" }}>✅ 待機中</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {tab === "home" && (
          <div style={{ padding: "18px 14px" }}>
            ...
            {vehicles.map((v, idx) => {
              const st     = STATUS[vehicleStatuses[v.id]];
              ...
              return (
                <div key={v.id}
                  ...
                  style={{
                    background: "#ffffff", border: `1px solid ${v.color}30`,
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    ...
                    <div style={{
                      ...
                      padding: "4px 12px", borderRadius: 20, border: `1px solid ${st.color}30`,
                      display: "flex", alignItems: "center", gap: 4
                    }}>
                      {ACTIVE_STATUSES.includes(vehicleStatuses[v.id]) && (
                        <span style={{ width: 5, height: 5, borderRadius: "50%", background: st.color, opacity: pulse ? 1 : 0.3, transition: "opacity 0.3s", display: "inline-block" }} />
                      )}
                      {st.label}
                    </div>
                  </div>
                    ...
                    <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
                      {crew.map(m => (
                        <div key={m.id} style={{
                          ...
                          <div style={{
                            width: 22, height: 22, borderRadius: "50%", background: v.color + "30",
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {actions.map(action => (
                      <button key={action.id}
                        onClick={() => { setModalVehicleId(v.id); setSelectedType(action.id); setShowModal(true); }}
                        style={{
                          background: `${action.color}10`, border: `1.5px solid ${action.color}40`,
                ...
                {[...messages].reverse().slice(0, 3).map(msg => {
                  ...
                  return (
                    <div key={msg.id} style={{
                      ...
                      padding: "10px 12px", marginBottom: 7, boxShadow: "0 1px 3px rgba(0,0,0,0.04)"
                    }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                        <span style={{ fontSize: 11, color: action ? action.color : "#64748b", fontWeight: 700 }}>
                          {vehicle?.icon} {action?.label || "メッセージ"}
                        </span>
        ...
        {tab === "chat" && (
          <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 148px)" }}>
            <div style={{ display: "flex", borderBottom: "1px solid #e2e8f0", background: "#ffffff" }}>
              {vehicles.map(v => (
                <button key={v.id} onClick={() => setChatVehicle(v.id)} style={{
                  flex: 1, padding: "10px 6px", cursor: "pointer", border: "none",
                  background: "transparent",
                  borderBottom: chatVehicle === v.id ? `2px solid ${v.color}` : "2px solid transparent",
                  fontSize: 12, fontWeight: 700,
                  color: chatVehicle === v.id ? v.color : "#94a3b8"
                }}>{v.icon} {v.label}</button>
              ))}
            </div>
            <div ref={chatRef} style={{ flex: 1, overflowY: "auto", padding: "14px", background: "#f8fafc" }}>
              {chatMessages.length === 0 && (
                <div style={{ textAlign: "center", color: "#cbd5e1", fontSize: 12, marginTop: 40 }}>まだメッセージはありません</div>
              )}
                ...
                return (
                  <div key={msg.id}>
                    ...
                    <div style={{ marginBottom: 9 }}>
                      <div style={{
                        background: msg.type !== "chat" ? (action ? `${action.color}12` : "#f1f5f9") : "#ffffff",
                        border: `1px solid ${msg.type !== "chat" && action ? action.color + "30" : "#e2e8f0"}`,
                        borderRadius: 12, padding: "9px 12px",
                        boxShadow: "0 1px 2px rgba(0,0,0,0.04)"
                      }}>
                        {msg.type !== "chat" && action && (
                          <div style={{ fontSize: 9, color: action.color, fontWeight: 700, marginBottom: 3 }}>{action.icon} {action.label}</div>
                        )}
            ...
            <div style={{ padding: "8px 10px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 7, background: "#ffffff" }}>
              <input value={chatInput} onChange={e => setChatInput(e.target.value)}
                ...
                placeholder={`${vehicles.find(v => v.id === chatVehicle)?.label}へ送信...`}
                style={{ flex: 1, background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 22, padding: "9px 14px", color: "#1e293b", fontSize: 13, outline: "none" }}
              />
              <button onClick={sendChat} style={{
                width: 38, height: 38, borderRadius: "50%",
                background: vehicles.find(v => v.id === chatVehicle)?.color || "#3b82f6",
                border: "none", cursor: "pointer", fontSize: 16, flexShrink: 0, color: "#fff"
              }}>↑</button>
            </div>
          </div>
        )}
        ...
        {tab === "history" && (
          <div style={{ padding: "18px 14px" }}>
            ...
            {[...messages].reverse().filter(m => m.type !== "chat").map(msg => {
              const action  = ALL_ACTIONS.find(a => a.id === msg.type);
              const vehicle = vehicles.find(v => v.id === msg.vehicleId);
              return (
                <div key={msg.id} style={{
                  background: "#ffffff", border: `1px solid ${action?.color}20`,
                  borderLeft: `3px solid ${action?.color || "#e2e8f0"}`,
            ...
            {messages.filter(m => m.type !== "chat").length === 0 && (
              <div style={{ textAlign: "center", color: "#cbd5e1", fontSize: 12, marginTop: 40 }}>履歴はありません</div>
            )}
          </div>
        )}
        ...
        {tab === "settings" && (
          <div style={{ padding: "18px 14px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 20 }}>
              {ALARM_TYPES.map(a => (
                <button key={a.id} onClick={() => {
                  setAlarmType(a.id);
                  // タップ時に試聴（AudioContextをユーザー操作で起動）
                  try {
                    if (!audioCtx.current) audioCtx.current = new (window.AudioContext || window.webkitAudioContext)();
                    if (audioCtx.current.state === "suspended") audioCtx.current.resume();
                  } catch(e) {}
                  ...
                  style={{
                    background: alarmType === a.id ? "#eff6ff" : "#f8fafc",
                    border: `1.5px solid ${alarmType === a.id ? "#3b82f6" : "#e2e8f0"}`,
            ...
            {[...vehicles, { id: null, label: "控え室", icon: "🏠", color: "#64748b", bg: "#f1f5f9" }].map(zone => (
              <div key={String(zone.id)} style={{
                background: "#f8fafc", border: `2px dashed ${zone.color}40`,
                      ...
                      {!isEditing ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{
                            ...
                            background: zone.color + "20", border: `2px solid ${zone.color}40`,
                            display: "flex", alignItems: "center", justifyContent: "center",
                            fontSize: 12, fontWeight: 700, color: zone.color, flexShrink: 0
                          }}>{member.avatar}</div>
                          <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: "flex", gap: 5 }}>
                            <button onClick={() => setAssigningMember(member.id)}
                              style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 7, padding: "4px 10px", cursor: "pointer", fontSize: 11, color: "#3b82f6", fontWeight: 600 }}>配置</button>
                            <button onClick={() => { setEditingMember(member.id); setEditForm({ name: member.name, role: member.role }); }}
                              style={{ background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 7, padding: "4px 10px", cursor: "pointer", fontSize: 11, color: "#64748b", fontWeight: 600 }}>編集</button>
                            <button onClick={() => setConfirmDelete(member.id)}
                              style={{ background: "#fff1f2", border: "1px solid #fecdd3", borderRadius: 7, padding: "4px 8px", cursor: "pointer", fontSize: 11, color: "#ef4444", fontWeight: 600 }}>削除</button>
                          </div>
                        </div>
                      ) : (
                        <div>
                          ...
                          <input value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))}
                            style={{ width: "100%", background: "#f8fafc", border: "1px solid #cbd5e1", borderRadius: 8, padding: "7px 10px", fontSize: 13, color: "#1e293b", outline: "none", marginBottom: 7, boxSizing: "border-box" }} />
                          <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 3 }}>役職</div>
                          <select value={editForm.role} onChange={e => setEditForm(f => ({ ...f, role: e.target.value }))}
                            style={{ width: "100%", background: "#f8fafc", border: "1px solid #cbd5e1", borderRadius: 8, padding: "7px 10px", fontSize: 13, color: "#1e293b", outline: "none", marginBottom: 10, boxSizing: "border-box" }}>
                            {ROLES.map(r => <option key={r}>{r}</option>)}
                          </select>
                          <div style={{ display: "flex", gap: 8 }}>
                            <button onClick={() => {
                              const avatar = editForm.name.trim().charAt(0) || member.avatar;
                              setMembers(ms => ms.map(m => m.id === member.id ? { ...m, ...editForm, avatar } : m));
                              setEditingMember(null);
                            }} style={{ flex: 1, background: "#3b82f6", border: "none", borderRadius: 8, padding: "8px", cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#fff" }}>保存</button>
                            <button onClick={() => setEditingMember(null)}
                              style={{ flex: 1, background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 8, padding: "8px", cursor: "pointer", fontSize: 12, color: "#64748b" }}>キャンセル</button>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                {members.filter(m => m.vehicleId === zone.id).length === 0 && (
                  <div style={{ textAlign: "center", fontSize: 11, color: "#cbd5e1", padding: "8px 0" }}>未配置</div>
                )}
              </div>
            ))}
            ...
            <button onClick={() => {
              ...
              setMembers(ms => [...ms, { id: newId, name: "新しい乗務員", role: "救急救命士", avatar: "新", vehicleId: null }]);
              setEditingMember(newId);
              setEditForm({ name: "新しい乗務員", role: "救急救命士" });
            }} style={{
              width: "100%", background: "#f0fdf4", border: "1.5px dashed #86efac",
      ...
      <div style={{
        ...
        borderTop: "1px solid #e2e8f0", display: "flex", padding: "7px 0 14px",
      ...
      {assigningMember && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 200 }}
            onClick={() => setAssigningMember(null)}>
            <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 430, padding: "22px 18px 34px", boxShadow: "0 -4px 24px rgba(0,0,0,0.12)" }}
              ...
              {zones.map(zone => (
                <button key={String(zone.id)} onClick={() => assignMember(assigningMember, zone.id)}
                  style={{
                    width: "100%", display: "flex", alignItems: "center", gap: 12,
                    background: member?.vehicleId === zone.id ? zone.bg : "#f8fafc",
                    border: `1.5px solid ${member?.vehicleId === zone.id ? zone.color : "#e2e8f0"}`,
                    borderRadius: 12, padding: "13px 16px", cursor: "pointer", marginBottom: 8
                  }}>
      ...
      {confirmDelete && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "0 24px" }}
            onClick={() => setConfirmDelete(null)}>
            <div style={{ background: "#fff", borderRadius: 16, padding: "24px 20px", width: "100%", maxWidth: 360, boxShadow: "0 8px 32px rgba(0,0,0,0.16)" }}
              onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setConfirmDelete(null)}
                  style={{ flex: 1, background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 10, padding: "11px", cursor: "pointer", fontSize: 13, color: "#64748b", fontWeight: 600 }}>キャンセル</button>
                <button onClick={() => { setMembers(ms => ms.filter(m => m.id !== confirmDelete)); setConfirmDelete(null); }}
                  style={{ flex: 1, background: "#ef4444", border: "none", borderRadius: 10, padding: "11px", cursor: "pointer", fontSize: 13, color: "#fff", fontWeight: 700 }}>削除する</button>
              </div>
            </div>
          </div>
        );
      ...
      {showModal && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }}
            onClick={closeModal}>
            <div style={{ background: "#ffffff", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 430, padding: "22px 18px 34px", boxShadow: "0 -4px 24px rgba(0,0,0,0.12)" }}
              onClick={e => e.stopPropagation()}>
              <div style={{ fontSize: 13, fontWeight: 700, color: vehicle?.color, marginBottom: 14 }}>
                {vehicle?.icon} {selectedType ? action?.label : "送信する内容を選択"}
              </div>
              {!selectedType ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {actions.map(a => (
                    <button key={a.id} onClick={() => setSelectedType(a.id)} style={{
                      background: `${a.color}08`, border: `1.5px solid ${a.color}30`,
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                    <input value={dispatchLocation} onChange={e => setDispatchLocation(e.target.value)}
                      placeholder="住所・場所名を入力　例：渋谷区道玄坂1-1"
                      style={{ flex: 1, background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 12, padding: "11px 13px", color: "#1e293b", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                    <button onClick={getCurrentLocation} style={{
                      flexShrink: 0, background: locationLoading ? "#f1f5f9" : "#f0fdf4",
                      border: "1px solid #86efac", borderRadius: 12, padding: "0 12px",
                      cursor: locationLoading ? "not-allowed" : "pointer",
                      fontSize: 18, color: locationLoading ? "#94a3b8" : "#16a34a"
                    }}>{locationLoading ? "⏳" : "📍"}</button>
                  </div>
                  {dispatchLocation && dispatchLocation.match(/^-?\d+\.\d+,-?\d+\.\d+$/) && (
                    <div style={{ fontSize: 10, color: "#16a34a", marginBottom: 6, display: "flex", alignItems: "center", gap: 4 }}>
                      ✅ 現在地を取得しました
                    </div>
                  )}
                  ...
                  <input value={dispatchNote} onChange={e => setDispatchNote(e.target.value)}
                    placeholder="例：3名負傷・意識あり"
                    style={{ width: "100%", background: "#f1f5f9", border: "1px solid #e2e8f0", borderRadius: 12, padding: "11px 13px", color: "#1e293b", fontSize: 13, outline: "none", marginBottom: 12, boxSizing: "border-box" }} />
                  <button onClick={() => sendDispatch(selectedType)} style={{
                    width: "100%", background: action?.color, border: "none", borderRadius: 12,
                    padding: "13px", cursor: "pointer", fontSize: 14, fontWeight: 700, color: "#fff", marginBottom: 8
                  }}>送信する</button>
                  <button onClick={() => setSelectedType(null)}
                    style={{ width: "100%", background: "none", border: "none", cursor: "pointer", color: "#94a3b8", fontSize: 12, padding: "6px" }}>← 戻る</button>
                </div>
              )}
            </div>
          </div>
