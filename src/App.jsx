...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { unsubStatus(); unsubMembers(); unsubMessages(); unsubVehicles(); };
  }, []);
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {tab === "settings" && (
          <div style={{ padding: "18px 14px" }}>
            ...
            <button onClick={() => { const newId = Math.max(...members.map(m => m.id)) + 1; const newMember = { id: newId, name: "新しい乗務員", role: "救急救命士", avatar: "新", vehicleId: null }; const newMembers = [...members, newMember]; fbSetMembers(newMembers); setEditingMember(newId); setEditForm({ name: "新しい乗務員", role: "救急救命士" }); }} style={{ width: "100%", background: "#f0fdf4", border: "1.5px dashed #86efac", borderRadius: 12, padding: "13px", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#16a34a" }}>＋ 乗務員を追加（控え室へ）</button>
          </div>
        )}
      </div>
