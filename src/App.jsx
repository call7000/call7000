...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAlert = onValue(alertRef, snap => {
      if (snap.exists() && snap.val().active === true) {
        setAlert(true);
        if (!alertTimer.current && audioCtx.current && audioCtx.current.state === "running") {
          setTimeout(() => {
            playAlertSound();
            alertTimer.current = setInterval(() => playAlertSound(), getAlarmDuration());
          }, 50);
        }
      } else if (snap.exists() && snap.val().active === false) {
        setAlert(false);
  ...
  function enableAudio() {
    try {
      ...
      // 短い無音を再生してAudioContextをアクティブに保つ
  ...
  function startAlert() {
    // Firebaseにアラートフラグを書き込む→全員の画面で検知して鳴動
  ...
  function stopAlert() {
    setAlert(false);
    if (alertTimer.current) { clearInterval(alertTimer.current); alertTimer.current = null; }
    // Firebaseのアラートフラグをリセット
    set(ref(db, "alertFlag"), { active: false, ts: Date.now() });
  }
